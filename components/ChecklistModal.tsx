...
/**
 * Generic "what shipped" modal for an activity group that renders as a single
 * clickable box in the feed (e.g. Quote Agent V1, CRO Works).
 */
export const ChecklistModal: React.FC<ChecklistModalProps> = ({ isOpen, onClose, title, items, period }) => {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-2xl transform overflow-hidden rounded-[3rem] border border-white/10 bg-[#0f1115]/80 backdrop-blur-3xl shadow-[0_50px_100px_-20px_rgba(0,0,0,0.8)] transition-all animate-in fade-in zoom-in duration-500">
        ...
        <div className="flex flex-col items-center pt-12 pb-8 relative border-b border-white/5">
          ...
          <h3 className="text-3xl font-black text-white uppercase tracking-tighter text-center px-12">{title}</h3>
          <p className="text-[11px] font-black text-violet-400 uppercase tracking-[0.3em] mt-2">{items.length} COMPLETED · {period}</p>
